...
/** Maps palette token keys → CSS custom property names on :root */
...
function writePaletteVars(tokens: PaletteTokens, target: HTMLElement = document.documentElement) {
  for (const [key, cssVar] of Object.entries(CSS_VAR_MAP) as [keyof PaletteTokens, string][]) {
    target.style.setProperty(cssVar, tokens[key])
  }

  // Semantic aliases used by Tailwind / components
...
/** Apply brand palette CSS variables. Call once at app boot (and when palette changes). */
