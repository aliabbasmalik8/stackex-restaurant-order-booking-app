...
import type { CardPaymentDriver, CardPaymentMeta, CardPaymentSession } from './types'
...
export function useCardPaymentSession({
  ...
  const prepare = useCallback(async () => {
    ...
    try {
      let intent
      try {
        intent = await createPaymentIntent(orderId)
      } catch (error) {
        setErrorMessage(
  ...
  const pay = useCallback(async (): Promise<'paid' | 'canceled' | 'failed'> => {
    ...
    try {
      ...
      try {
        const synced = await syncOrderPaymentStatus(orderId)
        ...
        setErrorMessage(getPaymentErrorMessage('payment.syncFailed', error, t))
        return 'failed'
      }
    } catch (error) {
      setErrorMessage(
