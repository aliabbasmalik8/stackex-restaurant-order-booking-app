import type { TranslationSchema } from './en'

export const ar: TranslationSchema = {
  common: {
    back: 'رجوع',
    change: 'تغيير',
    edit: 'تعديل',
    required: 'مطلوب',
    optional: 'اختياري',
    included: 'مشمول',
    aed: 'درهم',
    loading: 'جاري التحميل…',
    retry: 'إعادة المحاولة',
    close: 'إغلاق',
    cancel: 'إلغاء',
    delete: 'حذف',
    signIn: 'تسجيل الدخول',
    items: '{{count}} أصناف',
  },
  preview: {
    themeChip: 'المظهر',
    themeTitle: 'مظهر المعاينة',
    themeSubtitle: 'جرّب لوحة ألوان. يظهر هذا التحكم في المعاينة فقط.',
    palettes: {
      charcoal: 'فحمي',
      red: 'أحمر',
      dark: 'داكن',
      emerald: 'زمردي',
      saffron: 'زعفراني',
      midnight: 'منتصف الليل',
      olive: 'زيتوني',
    },
    paletteGroups: {
      warm: 'دافئ',
      cool: 'بارد',
      earth: 'ترابي',
      darkUi: 'واجهة داكنة',
    },
  },
  errors: {
    config_missing: {
      title: 'القائمة غير متاحة',
      message: 'تعذّر الاتصال بالمطعم الآن. حاول مرة أخرى بعد قليل.',
    },
    network: {
      title: 'لا يوجد اتصال',
      message: 'تحقق من الإنترنت وحاول مرة أخرى.',
    },
    permission: {
      title: 'تعذّر تحميل القائمة',
      message: 'حدث خطأ من جهتنا. حاول مرة أخرى بعد قليل.',
    },
    not_found: {
      title: 'غير موجود',
      message: 'هذا الصنف لم يعد متاحاً.',
    },
    empty: {
      title: 'لا يوجد شيء بعد',
      message: 'ستظهر القائمة عندما تصبح جاهزة.',
    },
    store_closed: {
      title: 'المتجر مغلق',
      message: 'لا نستقبل طلبات حالياً. يرجى المحاولة لاحقاً.',
    },
    item_unavailable: {
      title: 'صنف غير متاح',
      message:
        'بعض الأصناف في سلتك لم تعد متاحة وتمت إزالتها. راجع السلة وحاول مرة أخرى.',
    },
    branch_unavailable: {
      title: 'الفرع غير متاح',
      message:
        'هذا الفرع لا يستقبل طلبات حالياً. اختر فرعاً آخر أو حاول لاحقاً.',
    },
    out_of_delivery_range: {
      title: 'خارج نطاق التوصيل',
      message: 'لا نوصّل إلى هذا العنوان. اختر موقعاً أقرب إلى أحد مطابخنا.',
    },
    delivery_address_required: {
      title: 'العنوان مطلوب',
      message: 'أضف عنوان توصيل قبل إتمام الطلب.',
    },
    unknown: {
      title: 'حدث خطأ ما',
      message: 'حاول مرة أخرى. إذا استمر الأمر، عد لاحقاً.',
    },
  },
  store: {
    closedDefault: 'المتجر مغلق حالياً ولا نستقبل طلبات جديدة.',
    closedCta: 'مغلق',
    addUnavailable: 'الطلب غير متاح',
  },
  languages: {
    title: 'اللغة',
    subtitle: 'اختر لغة التطبيق.',
    en: 'الإنجليزية',
    ar: 'العربية',
    enNative: 'English',
    arNative: 'العربية',
    done: 'تم',
  },
  auth: {
    welcomeBack: 'أهلاً بعودتك',
    signInSubtitle: 'سجّل الدخول بالبريد الإلكتروني وكلمة المرور.',
    heroTitle: 'اطلب الآن. نوصّل إليك.',
    heroBody:
      'احفظ طلبك المعتاد، ادفع بنقرتين، واحصل على تنبيه في اللحظة التي يصبح فيها طعامك في الطريق.',
    heroEta: 'جاهز خلال ~{{minutes}} د',
    heroPickup: 'توصيل · نوصّله إلى بابك',
    signIn: 'تسجيل الدخول',
    continue: 'متابعة',
    resendEmail: 'إعادة إرسال البريد',
    orContinueWith: 'أو المتابعة عبر',
    apple: 'Apple',
    google: 'Google',
    newHere: 'جديد هنا؟',
    createAccount: 'إنشاء حساب',
    continueAsGuest: 'المتابعة كزائر ←',
    forgotPassword: 'نسيت كلمة المرور؟',
    forgotPasswordTitle: 'إعادة تعيين كلمة المرور',
    forgotPasswordSubtitle:
      'أدخل بريدك وسنرسل رابطاً لتعيين كلمة مرور جديدة.',
    sendResetEmail: 'إرسال رابط التعيين',
    passwordPlaceholder: 'كلمة المرور',
    password: 'كلمة المرور',
    confirmPassword: 'تأكيد كلمة المرور',
    confirmPasswordPlaceholder: 'أعد إدخال كلمة المرور',
    passwordMismatch: 'كلمتا المرور غير متطابقتين',
    showPassword: 'إظهار كلمة المرور',
    hidePassword: 'إخفاء كلمة المرور',
    signUpTitle: 'إنشاء حساب',
    signUpSubtitle: 'احفظ طلبك المعتاد وأكمل الدفع أسرع.',
    fullName: 'الاسم الكامل',
    email: 'البريد الإلكتروني',
    namePlaceholder: 'عائشة خالد',
    emailPlaceholder: 'aisha@example.com',
    loginEmailPlaceholder: 'test@example.com',
    loginPasswordPlaceholder: 'Preview123',
    phonePlaceholder: '50 555 0134',
    whatsappOffers: 'أرسل لي العروض والقائمة الجديدة على واتساب',
    createAccountCta: 'إنشاء حساب',
    termsPrefix: 'بالمتابعة فإنك توافق على ',
    terms: 'الشروط',
    and: ' و ',
    privacy: 'سياسة الخصوصية',
    passwordResetSent:
      'تم إرسال رابط التعيين — تحقق من البريد والرسائل غير المرغوب فيها.',
    alreadyHaveAccount: 'لديك حساب بالفعل؟',
    errors: {
      invalid_credential: 'البريد الإلكتروني أو كلمة المرور غير صحيحة.',
      account_not_exist: 'لا يوجد حساب بهذا البريد الإلكتروني.',
      email_in_use: 'يوجد حساب بهذا البريد بالفعل.',
      weak_password: 'استخدم كلمة مرور من 6 أحرف على الأقل.',
      invalid_email: 'أدخل بريداً إلكترونياً صالحاً.',
      too_many_requests: 'محاولات كثيرة. حاول مرة أخرى بعد قليل.',
      network: 'تحقق من الإنترنت وحاول مرة أخرى.',
      config_missing: 'تسجيل الدخول غير متاح الآن. حاول لاحقاً.',
      requires_recent_login: 'سجّل الدخول مرة أخرى لإدارة طرق الدخول.',
      credential_in_use: 'حساب Google هذا مرتبط بمستخدم آخر.',
      password_reset_not_allowed:
        'لا يمكن تحديث كلمة مرور حساب التجربة في وضع المعاينة',
      unknown: 'حدث خطأ ما. حاول مرة أخرى.',
    },
  },
  menu: {
    pickup: 'توصيل',
    eta: '١٥ د',
    etaMinutes: '{{minutes}} د',
    readyAround: 'جاهز ~{{time}}',
    searchPlaceholder: 'ابحث في القائمة',
    navLabel: 'القائمة',
    heroTitle: 'طازج من السيخ، جاهز خلال ١٥',
    heroSubtitle: 'اطلب الآن — نوصّل إلى بابك.',
    itemCount: '{{count}} أصناف',
    popular: 'شائع',
    addItem: 'أضف {{name}}',
    noResults: 'لا توجد أصناف تطابق هذا البحث.',
    viewOrder: 'عرض الطلب',
    addAddress: 'أضف عنواناً',
    addNewAddress: 'أضف عنواناً جديداً',
    addNewAddressCta: '+ أضف عنواناً جديداً',
    addressListTitle: 'العناوين المحفوظة',
    addressListEmpty: 'لا توجد عناوين محفوظة بعد',
    addressListEmptyHint: 'أضف موقعاً لنعرف أين نوصل الطلب.',
    addressDefault: 'الافتراضي',
    selectAddressFailed: 'تعذر تغيير العنوان. حاول مرة أخرى.',
    addressSearchPlaceholder: 'ابحث عن عنوان',
    addressSearchTitle: 'البحث عن عنوان',
    addressSearchHint: 'اكتب شارعاً أو منطقة أو معلماً.',
    addressSearchEmpty: 'لا توجد أماكن مطابقة لهذا البحث.',
    addressSearchFailed: 'تعذر البحث عن العناوين. حاول مرة أخرى.',
    addressDetailsTitle: 'تفاصيل العنوان',
    editAddressTitle: 'تعديل العنوان',
    addressLabel: 'حفظ كـ',
    addressLabels: {
      home: 'المنزل',
      work: 'العمل',
      other: 'آخر',
    },
    addressFloor: 'الطابق / الشقة',
    addressFloorPlaceholder: 'مثال: شقة ١٢، الطابق الثالث',
    addressNotes: 'ملاحظات التوصيل',
    addressNotesPlaceholder: 'رمز البوابة، معلم، توجيهات إضافية…',
    saveAddress: 'حفظ العنوان',
    saveAddressChanges: 'حفظ التغييرات',
    saveAddressFailed: 'تعذر حفظ هذا العنوان. حاول مرة أخرى.',
    updateAddressFailed: 'تعذر تحديث هذا العنوان. حاول مرة أخرى.',
    deleteAddressTitle: 'حذف العنوان',
    deleteAddressConfirm:
      'حذف هذا العنوان؟ سيُزال من قائمة العناوين المحفوظة.',
    deleteAddressFailed: 'تعذر حذف هذا العنوان. حاول مرة أخرى.',
    addressFound: 'تم العثور على العنوان',
    addressSheetTitle: 'عنوان التوصيل',
    useMyLocation: 'استخدم موقعي',
    confirmLocation: 'تأكيد هذا الموقع',
    continueLocation: 'متابعة',
    needLocationFirst: 'حدّد موقعك أولاً (بحث أو GPS أو ضع الدبوس).',
    addressLookupFailed: 'تعذر البحث عن هذا الموقع. حاول مرة أخرى.',
    locationDenied: 'يلزم إذن الموقع لوضع الدبوس.',
    locationUnavailable: 'تعذر قراءة موقعك. حاول مرة أخرى.',
    mapUnavailable:
      'الخريطة غير مُعدّة. عيّن VITE_GOOGLE_MAPS_WEB_KEY. البحث ما زال يعمل.',
    mapLoadFailed: 'تعذر تحميل خرائط Google. تحقق من المفتاح ثم أعد المحاولة.',
    mapHint: 'اضغط على الخريطة أو اسحب الدبوس. استخدم البحث للانتقال إلى مكان.',
    categories: {
      all: 'الكل',
    },
  },
  cart: {
    title: 'طلبك',
    empty: 'سلتك فارغة. أضف شيئاً لذيذاً من القائمة.',
    subtotal: 'المجموع الفرعي',
    vat: 'ضريبة 5%',
    total: 'الإجمالي',
    checkoutCta: 'الدفع · اختر وقتاً ←',
    openAria: 'عرض السلة',
    addPromo: 'أضف كود خصم',
    apply: 'تطبيق',
    promoUnavailable: 'لا توجد أكواد خصم على هذا الطلب حالياً.',
  },
  item: {
    addToOrder: 'أضف للطلب',
    cal: '{{count}} سعرة',
    specialInstructions: 'ملاحظات للمطبخ',
    specialInstructionsPlaceholder: 'مثال: اقطعه إلى نصفين، الصوص على الجانب…',
  },
  nav: {
    menu: 'القائمة',
    orders: 'الطلبات',
    signIn: 'تسجيل الدخول',
    profile: 'حسابي',
    openMenu: 'فتح القائمة',
  },
  orders: {
    title: 'الطلبات',
    subtitle: 'تتبّع طلبات التوصيل وأعد طلب مفضلاتك.',
    filterCurrent: 'الحالية',
    filterPrevious: 'السابقة',
    readyAround: 'جاهز حوالي {{time}}',
    reorder: 'أعد الطلب',
    trackOrder: 'تتبع الطلب',
    trackTitle: 'تتبع الطلب',
    backToOrders: 'العودة للطلبات',
    stepDone: 'تم',
    stepNow: 'جارٍ الآن',
    stepNext: 'التالي',
    cancelledHint: 'تم إلغاء هذا الطلب.',
    completedHint: 'تم توصيل هذا الطلب.',
    browseMenu: 'تصفح القائمة',
    pickup: 'توصيل',
    deliveryAddress: 'عنوان التوصيل',
    branch: 'الفرع',
    items: 'الأصناف',
    details: 'تفاصيل الطلب',
    placedOn: 'طُلب في {{date}}',
    emptyCurrentTitle: 'لا توجد طلبات بعد',
    emptyCurrentMessage: 'عندما تطلب للتوصيل سيظهر الطلب هنا.',
    emptyPreviousTitle: 'لا توجد طلبات سابقة',
    emptyPreviousMessage: 'الطلبات المكتملة ستظهر هنا.',
    status: {
      pending: 'قيد الانتظار',
      confirmed: 'مؤكد',
      preparing: 'قيد التحضير',
      ready: 'في الطريق',
      completed: 'مكتمل',
      cancelled: 'ملغى',
    },
  },
  profile: {
    title: 'حسابي',
    subtitle: 'أدِر بياناتك واللغة وطريقة تسجيل الدخول.',
    editTitle: 'تعديل الحساب',
    save: 'حفظ التغييرات',
    saved: 'تم الحفظ',
    sectionContact: 'التواصل',
    contactPhone: 'هاتف التواصل',
    emailReadOnly: 'البريد الإلكتروني',
    emailFromAuth: 'يُدار من حساب تسجيل الدخول',
    emailMissing: 'لا يوجد بريد إلكتروني على هذا الحساب.',
    language: 'اللغة',
    signInMethods: 'طرق تسجيل الدخول',
    signInMethodsSubtitle: 'كيف تدخل إلى هذا الحساب.',
    addPassword: 'إضافة',
    changePassword: 'تغيير',
    connectGoogle: 'ربط',
    googleConnected: 'مرتبط',
    addPasswordTitle: 'إضافة كلمة مرور',
    addPasswordSubtitle:
      'يمكنك أيضاً تسجيل الدخول بالبريد الإلكتروني وكلمة المرور هذه.',
    changePasswordTitle: 'تغيير كلمة المرور',
    changePasswordSubtitle: 'أدخل كلمة المرور الحالية ثم اختر واحدة جديدة.',
    currentPassword: 'كلمة المرور الحالية',
    newPassword: 'كلمة المرور الجديدة',
    savePassword: 'حفظ كلمة المرور',
    passwordAddHint: 'غير معيّنة — أضف واحدة للدخول بالبريد.',
    passwordSetHint: 'يمكنك الدخول بالبريد وكلمة المرور.',
    googleConnectHint: 'اربط Google بهذا الحساب.',
    googleConnectedHint: 'يمكنك الدخول عبر Google.',
    noFirebaseSession: 'سجّل الدخول مرة أخرى لإدارة طرق الدخول.',
    signOut: 'تسجيل الخروج',
    fallbackName: 'ضيف',
  },
  checkout: {
    title: 'الدفع',
    when: 'وقت التوصيل',
    asap: 'الآن',
    asapReady: 'جاهز ~{{time}} ({{minutes}} د)',
    schedule: 'جدولة',
    scheduleHint: 'اختر وقتاً اليوم',
    yourInfo: 'بياناتك',
    name: 'الاسم',
    phone: 'الهاتف',
    phoneRequired: 'رقم الهاتف مطلوب لتأكيد الطلب',
    address: 'العنوان',
    addressAdd: 'إضافة',
    addressMissing: 'أضف عنوان توصيل',
    addressRequired: 'العنوان مطلوب لتأكيد الطلب',
    whatsappHint: 'سنتواصل معك عند الجاهزية.',
    payment: 'طريقة الدفع',
    addCard: 'الدفع بالبطاقة',
    cash: 'الدفع عند التوصيل',
    cardDetailsNext:
      'تُدخل بيانات البطاقة بشكل آمن في الخطوة التالية. لا نحتفظ برقم البطاقة الكامل هنا.',
    summary: 'ملخص الطلب',
    placeOrder: 'تأكيد الطلب',
    placeOrderCta: 'تأكيد الطلب ←',
    placeOrderTotal: 'تأكيد الطلب · {{total}}',
    codeHint: 'ستحصل على رقم طلب لتتبع التوصيل',
    secureCheckout: 'دفع آمن',
    crumbCart: 'السلة',
    crumbPay: 'التوصيل والدفع',
    crumbDone: 'تم',
  },
  payment: {
    title: 'الدفع',
    subtitle: 'أكمل دفع البطاقة لتأكيد طلب التوصيل.',
    order: 'الطلب',
    orderNumber: 'طلب {{code}}',
    cardDetails: 'بيانات البطاقة',
    payNow: 'ادفع الآن',
    payAmount: 'ادفع {{total}}',
    chargedAs: 'يُخصم بـ {{currency}}',
    createIntentFailed: 'فشل إنشاء نية الدفع: {{detail}}',
    prepareFailed: 'فشل تجهيز نموذج الدفع: {{detail}}',
    confirmFailed: 'فشل تأكيد الدفع: {{detail}}',
    syncFailed: 'فشل مزامنة حالة الدفع: {{detail}}',
    notConfirmed: 'لم يتم تأكيد الدفع بعد. انتظر قليلاً ثم أعد المحاولة.',
    missingPublishableKey:
      'مدفوعات البطاقة غير مُعدّة. عيّن VITE_STRIPE_PUBLISHABLE_KEY.',
  },
  confirmation: {
    heroTitle: 'تم الطلب.\nقيد التحضير، {{name}}.',
    heroSubtitle:
      'احتفظ برقم الطلب. سنحدّث هذه الصفحة مع تقدّم الطلب — لا حاجة للتحديث.',
    pickupCode: 'رقم الطلب',
    readyAroundLabel: 'جاهز حوالي',
    readyAround: 'جاهز حوالي {{time}}',
    received: 'تم استلام الطلب',
    preparing: 'قيد تحضير طعامك',
    ready: 'في الطريق',
    justNow: 'الآن',
    happeningNow: 'جارٍ الآن',
    weWillNotify: 'سنُعلمك',
    directions: 'الاتجاهات',
    paidCard: 'مدفوع',
    paidCash: 'الدفع عند التوصيل',
    backToMenu: '← العودة للقائمة',
  },
  features: {
    previewUnavailable: 'غير متاح حالياً',
  },
}
