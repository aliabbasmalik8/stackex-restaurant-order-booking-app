...
const SETUP_HINT =
  'API is not reachable. Set VITE_API_URL in apps/web/.env (e.g. http://localhost:8000).'
...
function readApiPayload(error: unknown): {
  ...
  return {
    ...
    user_error_detail: user_error_detail ?? undefined,
  }
}
...
export function toAppError(error: unknown): AppError {
  ...
  if (code === 'ITEM_UNAVAILABLE') return wrap('item_unavailable')
  if (code === 'BRANCH_UNAVAILABLE') return wrap('branch_unavailable')
  if (code === 'OUT_OF_DELIVERY_RANGE') return wrap('out_of_delivery_range')
  ...
  if (/VITE_API_URL|not configured/i.test(message)) {
    if (import.meta.env.DEV) console.warn(`[api] ${SETUP_HINT}`)
